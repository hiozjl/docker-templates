...
if [[ $SERVICE_MANAGER == "openrc" ]]; then
    svc_enable() { rc-update add "$1"; }
    svc_start()  { rc-service "$1" start; }
    svc_restart(){ rc-service "$1" restart; }
    svc_active() { rc-service "$1" status &>/dev/null; }
fi
...
if ! command -v uuidgen &> /dev/null; then
    ...
    case $PKG_MANAGER in
        ...
        apt)  apt update && apt install -y uuid-runtime ;;
        yum)  yum install -y util-linux ;;
        *)    echo "无法检测系统类型，请手动安装 uuidgen。"; exit 1 ;;
    esac
    echo "uuidgen 已成功安装。"
fi
